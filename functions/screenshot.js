...
async function handler(event, context) {
	...
	let error_message = '';
	console.log('url = ', url);
	console.log('view_port = ', viewport);

	try {
		let valid = true;
		if (!isFullUrl(url)) {
			console.log(`ERROR_URL: Invalid \`url\`: ${url}`);
			valid = false;
		}

		if (!viewport || viewport.length !== 2) {
			console.log(
				`ERROR_API: Incorrect API usage. Expects one of: /:url/ or /:url/:size/ or /:url/:size/:aspectratio/`
			)
			valid = false;
		}
